...
public class PlayerModifier 
{
	...
	public static void main(String[] args) 
	{
		...
		while(true)
		{
			System.out.println("List Length: " + pm.player.size());
			String str;
	...
	public void showMessage()
	{
		...
		System.out.println("print - 把List所有內容寫入allPlayer.txt裡");
		System.out.println("add - 在list尾巴增加物件");
		System.out.println("search index(int) - 在Console顯示某一Player的資料");
		...
		System.out.println("ski index(int) indexForCharacterSkillEqu(int) indexForSkillList(int) - 將技能放入玩家中");
	}
	...
	public void readFile()
	{
		try 
		{
			...
			this.player = (List<Character>)reader.readObject();
			reader.close();
		}
		catch(Exception e) {e.printStackTrace();}
	}
	...
	public void writeFile()
	{
		try 
		{
			...
			writer.writeObject(this.player);
			writer.flush();
			writer.close();
		}
		catch(Exception e) {e.printStackTrace();}
	...
	public void print()
	{
		try 
		{		
			PrintWriter fileWriter = new PrintWriter(new FileOutputStream("./allPlayer.txt"));
			Character p;
	...
	public void changeHG(String str)
	{
		...
		try 
		{
			...
			if(index>=0 && index<player.size())
			{
				player.get(index).hGrow = num;
				player.get(index).setPropertybyFormula();
			}
		}
		catch(Exception e) {e.printStackTrace();}
	}
	...
	public void changeSP(String str)
	{
		...
		try 
		{
			...
			if(index>=0 && index<player.size())
			{
				player.get(index).getSpeed().setBaseSpeed(num); 
			}
		}
		catch(Exception e) {e.printStackTrace();}
	}
	public void changeSK(String str)
	{
		...
		try 
		{
			...
			if(index>=0 && index<player.size())
			{
				player.get(index).getSkillEquip().setSkill(indexEqu, this.skill.get(indexSki));
			}
		}
		catch(Exception e) {e.printStackTrace();}
	}
